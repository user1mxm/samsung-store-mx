  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.9 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }}
            className="fixed bottom-24 right-4 sm:right-6 w-[340px] max-w-[calc(100vw-2rem)] z-50 bg-white dark:bg-[#12121f] rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
      ...
      <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }} onClick={() => setOpen(!open)}
        className="fixed bottom-6 right-4 sm:right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-r from-[#1428A0] to-[#0077C8] text-white shadow-xl shadow-blue-900/30 flex items-center justify-center" title="Asistente IA Samsung">
